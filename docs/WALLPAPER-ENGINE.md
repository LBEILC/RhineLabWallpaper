...
> 当前行为（2026-09-25）：长期设置统一在 WE 壁纸属性调整，顶部提供「语言 / Language」与「启动默认模式」。页脚可临时切换模式、打开完整工作台及恢复自定义显示；重新加载回到 WE 默认配置。壁纸内设置弹窗已移除。字体许可随发行包保留。中英文细节见 [LOCALIZATION.md](LOCALIZATION.md)。下方旧日期的本地实验、临时设置及发布状态为历史记录，以最新条目为准。

## 页脚与按需工作台（2026-09-25）

WE 独立「页脚 | Footer」分组提供四项开关，默认开启：显示页脚信息、显示页脚时间、显示模式切换按钮、显示完整工作台按钮。页脚信息包括 SESSION、姓名及时间；3D、重播与两个快捷按钮属于操作入口，隐藏信息不会隐藏这些入口。页脚时间关闭时同时移除其分隔符。

「档案展示／桌面工作台」按钮直接切换模式。「打开完整工作台」临时显示全部工作台内容及五项功能，变为「恢复自定义显示」；再次点击恢复 WE 设置。全开期间修改 WE 配置会保留最新值，恢复时使用这些值。页脚设置独立于完整工作台。显式切换到档案展示会结束完整工作台状态。事项、计时和当前档案保留；重新加载清除临时模式与完整状态，重新读取 WE 默认值。

WE「工作台 · 区域位置」分别提供左侧时钟／事项、右侧功能面板、底部功能导航的水平／垂直滑杆，范围 −300～300px、默认 0；正值向右／向下。按屏幕实际像素移动，接近视口边缘时限制继续外移。与全局四边距叠加，底部导航仍共同响应下边距；位置变化不移动三维镜头。允许用户主动让区域靠近或重叠，不自动改写其他区域位置。窄屏沿用滚动布局，两个快捷按钮另占页脚一行。

关闭部分可用功能后，底部导航与右侧模块均按可用项连续编号；全关时隐藏内容及导航，完整工作台按钮仍可临时恢复全部功能。内容和功能设置在「默认工作台」或任一工作台快捷按钮开启时均可调整。

WE「声音」提供独立的「按钮点击音效」与「专注结束提示音」，默认开启，切换即时生效，并同时遵循总音效开关及音量。专注阶段正常倒数结束播放一次短提示；休息阶段不提醒，暂停、隐藏或重载后发现已经到期时不补响，重新开启提示也不补响。关闭工作台显示不会停止计时。新页脚按钮、3D 切换、事项及计时按钮使用短电子音；已有档案选择玻璃声仍由总音效控制，不叠加点击声。

音乐律动强度现在为 0–300%，默认 100%。原版、连续频谱、层叠流波均支持；0–200% 范围保留原有位移计算。效果与验证见 [工作台功能验证](../verification/WORKBENCH-CONTROLS.md)。
...
WE 顶部新增可折叠分组「登录身份」，其中的「登录人姓名」为文本输入，默认仍是 `JOYCE MOORE`。姓名同时用于开场身份确认行（`ID CONFIRMED : …` 仍在原片 321–339 帧窗口内逐字输入，长度只改变每字步长）、页脚姓名、设置页眉与档案访问记录；主机清空或只填空白时回落到默认姓名，最多取 24 个字符（按码点计数）。开场身份行拆成固定前缀与姓名两段：`ID CONFIRMED : ` 始终使用既有的 Novecento 轮廓字形；姓名的英文与数字等拉丁字符使用随包发行的同一套逐字轮廓图形（`scripts/make-name-glyphs.py`，167 个字符，含 Latin-1 字母与常用符号），因此英文名与原字体完全一致，中文等非拉丁字符改用 MiSans 实时文本，两者按同一基线对齐，同一姓名允许混排。页脚同理：默认姓名保留既有固定图形，拉丁姓名用同一套轮廓图形，非拉丁字符用 MiSans 实时文本。

「入场与画面 → 界面配色」下方新增「按时间自动切换明暗」及四个滑杆：切换为暗色的时／分、切换为亮色的时／分（默认 19:00 与 07:00，时 0–23、分 0–59），仅在开关打开时显示。调度按本机时间判断，暗色区间可以跨午夜；两个时间相同表示不自动切换，仍使用手动配色。到点后复用原有的逐张变色过渡，减少动态效果时直接切换。手动配色（WE 的下拉或壁纸内设置按钮）随时生效，但下一次跨越配置时间点会被自动结果覆盖；壁纸内设置页在开启时会显示当前时段说明。关闭自动切换即回到手动配色。

边界判断只在一秒一次的时间刻度上执行，且仅在计算出的目标与上次不同才应用，所以手动选择不会被同一时段内的下一次检查打断。

验证：`scripts/check-identity-theme.mjs` 覆盖姓名的空值／空白／控制字符／长度回落、页脚字形回退与转义、开场逐字进度、调度边界（含跨午夜、同时间、越界与非法宿主值）以及属性表默认值与条件；`scripts/check-identity-theme-host.mjs` 用真实 Wallpaper Engine 宿主窗口（关闭 3D 与开场，注入探针）验证属性默认值、开场自定义姓名与默认字形、运行时跨越边界、手动切换在下个时间点被覆盖、空白姓名回落与关闭开关后恢复手动配色，结果见 verification/identity-theme/host-results.json。
...
WE「入场与画面」新增独立的「模型精度」下拉选项，默认高档，即时生效。

| 档位 | 背景单档案三角形 | 选中／详情三角形 | 用途 |
| --- | ---: | ---: | --- |
| 高 | 4,200 | 82,348 | 原始完整细节 |
| 中 | 2,072 | 82,348 | 简化背景螺丝、边框，近景保持完整 |
| 低 | 1,554 | 21,869 | 进一步减少背景和近景细节，细小结构与高光可能变化 |

360° 拆解查看器保持完整模型。模型精度与画质、超级性能模式、减少动态效果分别控制；切换不改变已保存的其他设置。关闭 3D 后也可以更改档位，重新开启时应用。切换期间保留当前模型，加载失败给出提示，可重新选择重试。
