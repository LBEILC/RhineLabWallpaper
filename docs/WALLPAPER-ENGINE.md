...
WE 顶部新增可折叠分组「登录身份」，其中的「登录人姓名」为文本输入，默认仍是 `JOYCE MOORE`。姓名同时用于开场身份确认行（`ID CONFIRMED : …` 仍在原片 321–339 帧窗口内逐字输入，长度只改变每字步长）、页脚姓名、设置页眉与档案访问记录；主机清空或只填空白时回落到默认姓名，最多取 24 个字符（按码点计数）。默认姓名继续使用既有的 Novecento 固定字形，其他姓名改用实时文本（MiSans）显示，所以自定义姓名不需要重新导出字形。
