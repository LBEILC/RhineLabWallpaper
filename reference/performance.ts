...
Object.assign(window, { bench: {
  ...
  async prepare(name: string, options: any = {}) {
    ...
    scene.setMode(name === 'detail' ? 'detail' : 'archive');
  ...
  async sample(name: string, count = 48) {
    ...
    return {samples, stats:scene.getStats(), quality:document.querySelector('#scene')!.getAttribute('data-render-quality'),renderer:debug ? gl.getParameter(debug.UNMASKED_RENDERER_WEBGL) : gl.getParameter(gl.RENDERER),gpuTimer:!!ext};
  },
  draw() { step(); gl.finish(); },
} });
