...
vm.runInNewContext(readFileSync('wallpaper/host.js','utf8'),{window,Event,CustomEvent,location:{protocol:'file:'},performance});
...
assert.ok(Object.values(quiet).every(value => value === false), 'WE can hide every available element');
const clockOnly = applyVisibilityProperties(quiet, {showclock:{value:true}});
assert.equal(clockOnly.clock, true);
assert.equal(clockOnly.footer, false, 'Partial host changes preserve other hidden elements');
assert.ok(!('settings' in clockOnly), 'The removed settings entry has no visibility preference');
